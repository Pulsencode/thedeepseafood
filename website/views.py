...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        brand = Brand.objects.filter(status=True).exclude(name='Deep Sea').order_by('sequence')
        category = Category.objects.filter(status=True).order_by('-id')
        product = Product.objects.filter(brand__name='Deep Sea',status=True,homepage=True).order_by('sequence')[:9]
...
class IndexProductView(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                if type == 'All':
                    product = Product.objects.filter(status=True,brand__name='Deep Sea',homepage=True).order_by('sequence')[:9]
                    context = {'product': product}
                else:
                    product = Product.objects.filter(status=True,brand__name='Deep Sea',type= type,homepage=True).order_by('sequence')[:9]
...
class ProductView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if type:
            product = Product.objects.filter(brand__name='Deep Sea',type=type,status=True).order_by('sequence')
        else:
            product = Product.objects.filter(brand__name='Deep Sea',status=True).order_by('sequence')
        blog = BlogDetails.objects.filter(status=True).order_by('-id')
...
class ProductListingView(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                if type == 'All':
                    product = Product.objects.filter(status=True,brand__name='Deep Sea').order_by('sequence')
                else:
                    product = Product.objects.filter(status=True,brand__name='Deep Sea',type= type).order_by('sequence')
...
class ProductDetailsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cat = product.product_details.filter(status=True).order_by('category__sequence').values_list('category__id', flat=True).distinct().first()
        cat_name = product.product_details.filter(status=True).order_by('category__sequence').values_list('category__name', flat=True).distinct().first()
        ...
        try:
            ...
            data = None
        list = Product.objects.filter(status=True,brand__name='Deep Sea').exclude(slug_product=slug_product).order_by('sequence')[:6]
        category = Category.objects.filter(status=True,brand__name='Deep Sea').order_by('sequence')
        blog = BlogDetails.objects.filter(status=True).order_by('-id')
        context['blog'] = blog
        context['category'] = category
...
class SearchProductView(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                if type is not '' and type != 'All':
                    product = Product.objects.filter(status=True,brand__name='Deep Sea',type=type,name__icontains=input).order_by('sequence')

                # print(product)
                else:
                    product = Product.objects.filter(status=True,brand__name='Deep Sea',name__icontains=input).order_by('sequence')
...
class BrandView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        category = Category.objects.filter(brand__name='Oceano',status=True).order_by('sequence')
        first_category = category.first()
        products = Product.objects.filter(status=True,brand__name='Oceano',product_details__category=first_category).order_by('sequence')[:4]
        total_product = Product.objects.filter(status=True,brand__name='Oceano',product_details__category=first_category).order_by('-id').count()
        blog = BlogDetails.objects.filter(status=True).order_by('-id')
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                ...
                products = Product.objects.filter(status=True,brand__name='Oceano',product_details__category__name=type).order_by('sequence')[:4]
                total_count =Product.objects.filter(status=True,brand__name='Oceano',product_details__category__name=type).order_by('-id').count()
                ...
                template = loader.get_template(self.template_name)
                html_content = template.render(context, request)
                return JsonResponse({'status': True, 'template': html_content, 'total_count': total_count})
            except Product.DoesNotExist:
                return JsonResponse({'error': 'Product not found'}, status=404)
        else:
            return JsonResponse({'error': 'Invalid request'}, status=400)
...
def load_more_product(request):
    try:
        ...
        total_count = Product.objects.filter(status=True,brand__name='Oceano',product_details__category__name=type).order_by('-id').count()
        # print(total_count,'***')
        data = Product.objects.filter(status=True,brand__name='Oceano',product_details__category__name=type).order_by('sequence')[offset:offset + limit]
        
        t = render_to_string('website/brands/product_load.html', {'data': data, 'total_count': total_count})
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                ...
                print(products)
                cat = products.product_details.filter(status=True).values_list('category__id', flat=True).order_by('-id').distinct().first()
                print(cat)
                try:
                    data = ProductDetails.objects.get(product=products, category=cat)
                except ProductDetails.DoesNotExist:
...
class LoadEvents(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                gallery = EventGallery.objects.filter(status=True).order_by('sequence')[:3]
                total_events = EventGallery.objects.filter(status=True).count()
                context = {
                    'gallery': gallery,
...
class LoadNews(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                news = NewsDetails.objects.filter(status=True,type='Company News').order_by('sequence')[:3]
                total_news = NewsDetails.objects.filter(status=True,type='Company News').count()
                context = {
                    'news': news,
                    'type': 'Company News',
                ...
                template = loader.get_template(self.template_name)
                html_content = template.render(context, request)
                return JsonResponse({'status': True, 'template': html_content,'total_news':total_news})
            except Exception as e:
                print(str(e))
                return JsonResponse({'error': str(e)}, status=500)
...
class LoadGlobalNews(View):
    ...
    def get(self, request):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            ...
            try:
                news = NewsDetails.objects.filter(status=True,type='Global News').order_by('sequence')[:3]
                total_news = NewsDetails.objects.filter(status=True,type='Global News').count()
                context = {
                    'news': news,
                    'type': 'Global News',
                    'total_news':total_news
...
def load_more_news(request):
    ...
    limit = int(request.GET['limit'])
    type = request.GET['type']
    data = NewsDetails.objects.filter(status=True,type=type).order_by('sequence')[offset:offset+limit]
    total_news = NewsDetails.objects.filter(status=True, type=type).count()
    t=render_to_string('website/news/loadmore_news.html',{'data':data,'type': type,'total_news': total_news})
...
def load_more_events(request):
    ...
    limit = int(request.GET['limit'])
    # type = request.GET['type']
    data = EventGallery.objects.filter(status=True).order_by('sequence')[offset:offset+limit]
    total_events = gallery = EventGallery.objects.filter(status=True).count()
    t=render_to_string('website/news/loadmore_events.html',{'data':data,'total_events': total_events})
    return JsonResponse({'data': t})     
...
class PromotionView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data = PromotionDetails.objects.filter(status=True).exclude(id=id).order_by('-id')
        context['data'] = data
...
class BlogDetailsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        blog = BlogDetails.objects.filter(status=True).exclude(slug=slug).order_by('-id')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if honey:
            ...
            if name and location and phone and email:
                contact = ContactUsDetails(name=name,email=email,location=location,mobile_no=phone,message=message)
                contact.save()
...
class ProductEnquiryView(View):
    def post(self, request, *args, **kwargs):
        ...
        if honey:
            ...
            # Honey field is empty, proceed with sending the email
            if name and location and phone and email:
                enquiry = EnquiryDetails(product=product,name=name,email=email,location=location,mobile_no=phone,message=message)
                enquiry.save()
                subject = 'New Form Submission-Deep Seafood Company Website'
